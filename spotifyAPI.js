...
var client_id = '462b6027dec7432880e39282ee49b240';
var client_secret = 'feb817257c334057a3fc7c38bead315d';
...
const getSongs = (offset) => {
    return new Promise((resolve, reject) => {
        spotifyApi.getMySavedTracks({
            ...
            if (error.statusCode === 401) { // Se il token è scaduto
                console.log('Access token expired, refreshing...');
                spotifyApi.refreshAccessToken()
                    .then(function(data) {
                        console.log('The access token has been refreshed!');
                        spotifyApi.setAccessToken(data.body['access_token']); // Aggiorna il token
...
const getFeatures = (trackIds) => {
    return new Promise((resolve, reject) => {*//*
        ...
        spotifyApi.getAudioFeaturesForTracks(trackIds)
            ...
            .catch(error => {
                console.log("Error (line 129): " + JSON.stringify(error)); // Mostra dettagli completi sull'errore
                reject(error); // Gestisci l'errore, se necessario
            });
    });
};
*/
                ...
                if (error.statusCode === 401) { // Se il token è scaduto
                    ...
                    spotifyApi.refreshAccessToken()
                        .then(function(data) {
                            console.log('The access token has been refreshed!');
                            spotifyApi.setAccessToken(data.body['access_token']); // Aggiorna il token
                            return spotifyApi.getAudioFeaturesForTracks(trackIds); // Riprova la richiesta
                        })
                        .then(data => {
                            resolve(data.body.audio_features);
                        })
